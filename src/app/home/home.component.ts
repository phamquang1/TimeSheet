...
export class HomeComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit(): void {
        let fDate = moment().startOf('isoWeek').add(this.activeView, 'w').format('MM-DD-YYYY');
        let tDate = moment().endOf('isoWeek').add(this.activeView, 'w').format('MM-DD-YYYY');
    ...
    //thay đổi hiển thị thời gian bắt đầu và kết thúc 
    ...
    changeView(reset?: boolean, fDate?: any, tDate?: any) {
        ...
        if (this.viewChange.value == this.APP_CONSTANT.TypeViewHomePage.Week) {
            ...
            toDate = moment().endOf('isoWeek').add(this.activeView, 'w');
            this.typeDate = 'Week';
        }
        if (this.viewChange.value == this.APP_CONSTANT.TypeViewHomePage.Month) {
            fromDate = moment().add(this.activeView, 'M').startOf('M');
            toDate = moment().add(this.activeView, 'M').endOf('M');
            this.typeDate = 'Month';
        }
        if (this.viewChange.value == this.APP_CONSTANT.TypeViewHomePage.Quater) {
            fromDate = moment().startOf('Q').add(this.activeView, 'Q');
            toDate = moment().endOf('Q').add(this.activeView, 'Q');
        ...
        if (this.viewChange.value == this.APP_CONSTANT.TypeViewHomePage.Year) {
            ...
            toDate = moment().endOf('y').add(this.activeView, 'y');
            this.typeDate = 'Years'
        }
    ...
    showPopup(): void {
        let popup = this._dialog.open(PopupsComponent);
